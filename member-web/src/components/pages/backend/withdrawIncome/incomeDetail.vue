    ...
    <div>
            ...
            <div class="col-md-12">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb shadow">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">ถอนรายได้</li>
                    </ol>
                </nav>
            </div>
        </div>
        ...
        <div class="card mt-3 shadow p-3 text-center border-0">
            <table style="width: 100%;">
                <tr>
                    <td colspan="6" style="text-align: center; padding-bottom: 16px"><h5>ถอนรายได้</h5></td>
                </tr>
                <tr>
                    <td style="text-align: right; padding-right: 20px"><img id="imgBank" src="" /></td>
            ...
            <form v-on:submit.prevent="fnWithdrawIncome">
                <div class="form-group">
                    ...
                    <label class="mt-2 text-danger">** เพื่อสิทธิประโยชน์ของผู้ใช้ ระบบมีความจำเป็นต้องให้ผู้ใช้ ถอนเงินออกจากเครดิตที่เลือกทั้งหมด แล้วระบบจะโอน เข้าบัญชีของคุณภายใน 10 นาที</label>

                    <button class="btn btn-blue btn-lg text-center text-white" style="border-radius: 25px;" id="btnStatus">
                        <i class="fas fa-check-circle"></i> ยืนยันการถอนรายได้
                    </button>
                    <br>
                    <label class="mt-2 text-danger">** ต้องมีรายได้ขั้นต่ำ {{ cal }} บาท**</label>
                </div>
            </form>
        </div>
    </div>
...
export default {
    ...
    methods: {
        async getDataWithdrawIncome() { // select data
            const rs = await withdrawService.getDataWithdrawIncome();
            this.withdrawIncome = rs.income
            this.cal = rs.setting.amount_withdraw.toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')
            ...
            if (rs.cal >= rs.setting.amount_withdraw) {
                ...
                document.getElementById("btnStatus").setAttribute("class", "btn btn-danger btn-lg text-center text-white");
            }
        },
        async fnWithdrawIncome() { // ถอนรายได้
